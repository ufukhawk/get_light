...
// ============================================================
// FastList - A lightweight linked list for managing subscribers
// ============================================================
...
class FastList<T> {
  ...
  void addListener(MiniSubscription<T> data) {
    var newNode = Node<MiniSubscription<T>>(data: data);
    if (isEmpty) {
      _head = _tail = newNode;
    } else {
      _tail!.next = newNode;
...
// ============================================================
// MiniStream - Lightweight reactive stream (no dart:async Streams)
// ============================================================

class MiniStream<T> {
  FastList<T> _listenable = FastList<T>();
  late T _value;
  ...
  void close() {
    if (_isClosed) {
      throw StateError('You cannot close a closed Stream');
    }
